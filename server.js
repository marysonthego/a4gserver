...
server.post("/register", async (req, res) => {
    let { firstName, lastName, email, cell, addr1, addr2, city, st, zip, county, password } = req.body;
            ...
            (err, results) => {
                ...
                if (results.rows.length > 0) {
                    ...
                    pool.query(
                        ...
                        [firstName, lastName, email, cell, addr1, addr2, city, st, zip, county, hashedPassword],
                        (err, results) => {
                            if (err) {
                                throw err;
                            }
                            console.log(results.rows);
                            req.flash("success_msg", "Registration was successful, please login");
                            res.redirect("/login");
                        }
                    )
                }
